...
要切换配置，可以右键单击工程名选择 C/C++ Project Settings 打开工程属性配置界面

image::Snipaste_2026-01-09_10-23-42.png[role="thumb", width=480]

在弹出的对话框中选择 C/C++ 构建 -> 设置，然后点击管理配置

image::Snipaste_2026-01-09_10-26-05.png[role="thumb", width=800]

在弹出的对话框中选中 Debug，然后选择设置为活跃，最后点击确定，切换到 Debug 配置

image::Snipaste_2026-01-09_10-27-57.png[role="thumb", width=400]

点击工程属性配置界面下方的 `应用并关闭` 

image::Snipaste_2026-01-09_10-41-17.png[role="thumb", width=800]

== 构建工程

点击 e2 studio 左上角的锤子图标构建工程。下方控制台会输出编译信息。

image::Snipaste_2026-01-09_10-39-18.png[role="thumb", width=800]

== RTT View 连接

在 Debug 配置下，工程将 C 标准库的标准输入输出流用 RTT View 实现，需要用 RTT Viewer 来查看输出.
本样例工程还设置了输出语言为中文，会有部分输出通过UTF-8编码输出。而 SEGGER 自带的那个PC端应用程序 RTT Viewer 不支持UTF编码，因此需要使用第三方的工具通过 telnet 的方式连接，这里先描述如何使用 telnet 连接再进行调试。或者参照 <<语言配置>> 一节将语言设为英文，这样可以直接使用 SEGGER 自带的 RTT Viewer。

* 打开 map 文件，这个文件在工程目录的 `配置名` 目录下，当前已经将配置设为了 Debug，因此要去 Debug 目录寻找 map 文件。如果后续想换配置，如换成 Release，那就应该去 Release 目录寻找文件。
...
Keil MDK 不区分 Debug 和 Release 配置，先使用 RASC 生成，然后修改编译器参数即可。打开 RASC，点击左上角的 `File` -> `Open`，然后选择到工程目录下的 `configuration.xml`
...
这里分数相比于 xref:perf_counter_cpkhmi_ra8p1_ep_readme.adoc#快速查看测试结果[快速查看测试结果] 中预留的 hex 文件运行结果较低，是因为 RASC 每次生成都会把优化配置覆盖为默认。默认以减少空间占用优先，要修改优化配置，可以参考 xref:perf_counter_cpkhmi_ra8p1_ep_readme.adoc#修改编译优化选项[修改编译优化选项]
...
=== DCache
DCache 缓存可以有效加速数据的访问，该功能需要在 FSP COnfiguration 中开启。在 FSP Configuration 配置界面选择 BSP 选项卡，然后在下方窗口点击属性，展开 `RA8P1 Family` -> `Cache settings` 中将 `Data cache` 设为 `Enable`，`Data cache forced write-through` 保留为 `Disabled`
