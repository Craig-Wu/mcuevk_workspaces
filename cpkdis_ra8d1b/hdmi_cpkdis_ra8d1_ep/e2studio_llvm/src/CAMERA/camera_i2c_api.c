...
fsp_err_t camera_i2c_comm_write(uint32_t sub_address, uint32_t sub_address_length, const uint8_t *data, size_t data_length)
{
    ...
    // Write I2C data
    err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, buffer, buffer_length, false);
...
//    APP_ERR_RETURN(err, " ** R_IIC_MASTER_Write API FAILED ** \r\n");
    ...
    // Write register index
    err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, &buffer[0], sub_address_length, false);   //Stop for OV7725
    APP_ERR_RETURN(err, " ** R_IIC_MASTER_Write API FAILED ** \r\n");
    ...
    // Read data
    err = R_IIC_MASTER_Read(&g_i2c_master_ctrl, data, data_length, false);
