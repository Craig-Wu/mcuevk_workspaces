### 工程概述:
- 该示例工程演示了基于cpkhmi-ra8d1如何恢复touch功能，主要针对gt911系列的触摸芯片。用于不小心把触摸芯片的配置数据擦除或者修改的情况。

### 针对10.1inch 屏，800*1280像素：

注意取消 #define WRITE_GT11_FW 的注释，打开这个宏后，代码会通过I2C把以下配置内容写到touch芯片。
...
### 针对其他的touch芯片，不小心擦除或者修改了配置数据，可以通过读取一个正常的touch芯片配置数据，然后修改 g911xFW 的值，再重新写回去。
可以取消 #define DUMP_GT911_REGS 的注释，然后执行init_ts();和enable_ts();。读回来的数据会存在 g_read_config 数组。
 ...
 **********************************************************************************************************************/
volatile bool is_touch_pressed = false;
