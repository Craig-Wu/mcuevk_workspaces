...
static void make_cpkexp_init (void)
{
  ...
  //  err =R_SCI_B_UART_Open (&g_uart_ctrl, &g_uart_cfg)
    //R_PFS->PORT[0].PIN[0].PmnPFS_b.PODR;
    //static void r_ioport_pfs_write (bsp_io_port_pin_t pin, uint32_t value)
    R_BSP_PinAccessEnable();
    R_PFS->PORT[4].PIN[0].PmnPFS_b.PSEL=0X00;
    R_PFS->PORT[4].PIN[1].PmnPFS_b.PSEL=0X02;
    //r_ioport_pfs_write(BSP_IO_PORT_04_PIN_00,0);
   // r_ioport_pfs_write(BSP_IO_PORT_04_PIN_01,2);
