...
#define SDRAM_SIZE                  ((uint32_t)0x1000000UL)     // 16MBytes
#define SDRAM_BANK_ADDR             ((uint32_t)0x68000000UL)    // SDRAM Address space
...
volatile uint32_t SRAM_write_buff_Cache[SDRAM_EXAMPLE_DATALEN]  BSP_PLACE_IN_SECTION("__ram_zero$$");
volatile uint32_t SRAM_read_buff_Cache[SDRAM_EXAMPLE_DATALEN]  BSP_PLACE_IN_SECTION("__ram_zero$$");

volatile uint32_t SRAM_write_buff_Nocache[SDRAM_EXAMPLE_DATALEN] BSP_PLACE_IN_SECTION("__ram_zero_nocache$$");
volatile uint32_t SRAM_read_buff_Nocache[SDRAM_EXAMPLE_DATALEN]  BSP_PLACE_IN_SECTION("__ram_zero_nocache$$");

volatile uint32_t dtcm_write_buffer[SDRAM_EXAMPLE_DATALEN/4] BSP_PLACE_IN_SECTION("__dtcm_zero$$");
volatile uint32_t dtcm_read_buffer[SDRAM_EXAMPLE_DATALEN/4] BSP_PLACE_IN_SECTION("__dtcm_zero$$");


volatile uint32_t sdram_cache[SDRAM_EXAMPLE_DATALEN] BSP_PLACE_IN_SECTION("__sdram_zero$$");
volatile uint32_t sdram_nocache[SDRAM_EXAMPLE_DATALEN]  BSP_PLACE_IN_SECTION("__sdram_zero_nocache$$");
...
// full range read/write test;
        APP_PRINT("**********Full range SRDAM read/write test start! **********\r\n");
        for (uint32_t i = 0; i < SDRAM_SIZE / 4; i++)
        {

            *(__IO uint32_t *)(SDRAM_BANK_ADDR + i * 4) = i ^ (uint32_t)0x5A5AA5A5;
        ...
        for (uint32_t i = 0; i < SDRAM_SIZE / 4; i++)
        {
            ...
            data = *(__IO uint32_t *)(SDRAM_BANK_ADDR + i * 4);
            if (data != (i ^ 0x5A5AA5A5))
            {
                APP_PRINT("**********Full range SRDAM test failed **********\r\n");
                while(1)
                {
                    ;
                }
        ...
        APP_PRINT("**********Full range SRDAM read/write test Pass! **********\r\n");
#endif
