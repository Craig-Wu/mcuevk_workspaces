...
如果您还不是本代码仓库的合作开发者，请联系craig.wu.wx@renesas.com
...
2. 创建工程。您可以在本地任意目录（例如e2studio的缺省Workspace目录）创建工程，也可以直接在本地代码仓库的存储位置创建工程。
   ...
   如果您创建的是RA8 MCU的示例工程，请尽量选择最新版本的FSP，并使用LLVM编译器。

   如果您使用CPK开发板，请安装CPK板对应的BSP，并基于此BSP进行后续开发。
   每个CPK板的BSP文件，请参考CPK板使用手册，或对应cpk样例程序目录下的readme。
...
5. 如有必要，请编写详细的示例工程及样例代码说明文件和使用教程，文件名为`样例代码目录名_readme.md`或`样例代码目录名_readme.adoc`，图片统一存放在images目录下。下图为QSPI Flash样例代码目录下的说明文件
...
- 将编译生成的可直接运行的二进制文件（Hex，Mot等）放到项目根目录下
- 导出项目（仅导出必要的项目文件）
  * 如果您不是在本地仓库目录直接开发，请先将示例工程导出到本地仓库对应的位置
- 更新readme.md，标注样例工程开发已完成，并推送样例代码到远程仓库。   
