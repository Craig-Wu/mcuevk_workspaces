...
fsp_err_t ospi_b_init (void)
{
 ...
 //mask for Winbond QSPI Flash W35T51NW 
...
fsp_err_t ospi_b_set_protocol_to_opi (void)
{
    ...
    else if(SPI_FLASH_PROTOCOL_EXTENDED_SPI == g_ospi_b_ctrl.spi_protocol)
    {
        ...
        err = R_OSPI_B_DirectTransfer (&g_ospi_b_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
        APP_ERR_RETURN(err, "set the volatile configuration register ODDR mode FAILED\r\n");
        ...
        /* Change the OCTACLK clock to 100 MHz in DDR mode */
        octaclk.source_clock = BSP_CLOCKS_SOURCE_CLOCK_PLL2P;
        ...
        err = R_OSPI_B_DirectTransfer (&g_ospi_b_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
        APP_ERR_RETURN(err, "read volatile configuration register FAILED\r\n");
        // Write the flash with manual command in ODDR mode
        memset(&transfer, 0, sizeof(transfer));
        transfer.command        = 0x1212;
        transfer.command_length = 0x02;
        transfer.address        = 0x2000;
        transfer.address_length = 0x04;
        transfer.data_u64       = 0x122355AA55bbccdd;
        transfer.data_length    = 0x08;
        transfer.dummy_cycles   = 0x00;
        err = R_OSPI_B_DirectTransfer (&g_ospi_b_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
        APP_ERR_RETURN(err, "wirte Flash FAILED\r\n");
          // read the flash with manual command in ODDR mode
        memset(&transfer, 0, sizeof(transfer));
        transfer.command        = 0x0C0C;
        transfer.command_length = 0x02;
        transfer.address        = 0x2000;
        transfer.address_length = 0x04;
        transfer.data           = 0x00;
        ...
        transfer.dummy_cycles   = 0x08;
        err = R_OSPI_B_DirectTransfer (&g_ospi_b_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
        APP_ERR_RETURN(err, "read Flash FAILED\r\n");
